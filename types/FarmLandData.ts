...
export class FarmLandData {
    ...
    public static fromContract(contract: Contract): FarmLandData {
        ...
        const patronLevel = BurstValue.fromPlanck(view.getVariableAsDecimal(Addresses.PatronLevel))
        const farmValue = BurstValue.fromPlanck(view.getVariableAsDecimal(Addresses.FarmValue))
        const farmers = Addresses.Farmers.map<string>(view.getVariableAsDecimal.bind(view))
        const lords = Addresses.Lords.map<string>(view.getVariableAsDecimal.bind(view))
        const lordInvestments = Addresses.LordsInvestments.map<BurstValue>(address => BurstValue.fromPlanck(view.getVariableAsDecimal(address)))
        const margin = Number.parseInt(view.getVariableAsDecimal(Addresses.Margin)) - 100
        ...
        const winnerFarmerBalance = BurstValue.fromPlanck(view.getVariableAsDecimal(Addresses.WinnerFarmer))
